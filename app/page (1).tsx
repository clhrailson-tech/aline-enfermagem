...
const PIN_IDENTITIES: Record<LoginProfile, string> = {
  aline: "aline@aline-enfermagem.app",
  admin: "admin@aline-enfermagem.app",
};
...
async function signInWithPin(pin: string, profile: LoginProfile) {
  try {
    const result = await authRequest<AuthTokenResponse>("token?grant_type=password", { email: PIN_IDENTITIES[profile], password: pin });
    return normalizeSession(result);
  } catch {
    throw new Error("PIN incorreto. Confira e tente novamente.");
  }
}
...
export default function Home() {
  ...
  async function handleLogin(pin: string, profile: LoginProfile) {
    const session = await signInWithPin(pin, profile);
    setAuthSession(session);
    saveStored(AUTH_STORAGE_KEY, session);
  }
...
function LoginScreen({ onLogin }: { onLogin: (pin: string, profile: LoginProfile) => Promise<void> }) {
  const [pin, setPin] = useState("");
  const [profile, setProfile] = useState<LoginProfile>("aline");
      ...
      <section className="relative w-full max-w-md rounded-[2rem] border border-white/70 bg-white/95 p-7 shadow-2xl shadow-[#183c2f]/10 md:p-9">
        ...
        <form onSubmit={submit} className="space-y-5">
          <fieldset className="space-y-2">
            <legend className="text-sm font-bold">Quem vai entrar?</legend>
            <div className="grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setProfile("aline")} className={`rounded-xl border px-3 py-3 text-sm font-bold transition ${profile === "aline" ? "border-[#388e3c] bg-[#edf8ee] text-[#255f29] ring-1 ring-[#388e3c]" : "border-slate-200 bg-white text-slate-500 hover:bg-slate-50"}`}>Aline</button>
              <button type="button" onClick={() => setProfile("admin")} className={`rounded-xl border px-3 py-3 text-sm font-bold transition ${profile === "admin" ? "border-[#388e3c] bg-[#edf8ee] text-[#255f29] ring-1 ring-[#388e3c]" : "border-slate-200 bg-white text-slate-500 hover:bg-slate-50"}`}>Administrador</button>
            </div>
          </fieldset>
